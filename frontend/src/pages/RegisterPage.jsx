...
// ============================================================
//  RegisterPage — inputs contrôlés, validation manuelle
//  Plus de react-hook-form ni zodResolver (source des bugs)
// ============================================================
...
function TextInput({ type = 'text', value, onChange, placeholder, autoComplete, error }) {
...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({ firstName:'', lastName:'', email:'', password:'', role:'both' })
  ...
  const validate = () => {
    const e = {}
    if (form.firstName.trim().length < 2)  e.firstName = 'Prénom requis (2 caractères minimum)'
    if (form.lastName.trim().length < 2)   e.lastName  = 'Nom requis (2 caractères minimum)'
      ...
      <div className="flex flex-col justify-center bg-yoba-gray px-8 py-12 lg:px-20 overflow-y-auto">
        ...
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.45 }}>
          ...
          <div className="flex bg-white border border-brown/10 rounded-lg p-1 mb-6">
            <Link to="/connexion" className="flex-1 py-2.5 text-center text-yoba-gray-dark font-condensed font-bold text-sm tracking-widest uppercase hover:text-brown transition-colors">
              Connexion
            </Link>
            <button type="button" className="flex-1 py-2.5 rounded-md bg-brown text-white font-condensed font-bold text-sm tracking-widest uppercase">
              Inscription
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Prénom" error={errs.firstName}>
                <TextInput type="text" value={form.firstName} onChange={setField('firstName')}
                           placeholder="Aminata" autoComplete="given-name" error={errs.firstName} />
              </Field>
              <Field label="Nom" error={errs.lastName}>
                <TextInput type="text" value={form.lastName} onChange={setField('lastName')}
                           placeholder="Koné" autoComplete="family-name" error={errs.lastName} />
              </Field>
            </div>

            <Field label="Adresse email" error={errs.email}>
              <TextInput type="email" value={form.email} onChange={setField('email')}
                         placeholder="vous@example.com" autoComplete="email" error={errs.email} />
            </Field>

            <Field label="Mot de passe" error={errs.password}>
              <TextInput type="password" value={form.password} onChange={setField('password')}
                         placeholder="8 caractères minimum" autoComplete="new-password" error={errs.password} />
            </Field>
                ...
                {ROLES.map(r => (
                  <label key={r.value}
                         className={`flex items-center gap-3 p-3 bg-white border rounded-lg cursor-pointer transition-colors
                           ${form.role === r.value ? 'border-brown bg-brown-ultra' : 'border-brown/15 hover:border-brown'}`}>
                    <input type="radio" name="role" value={r.value}
                           checked={form.role === r.value} onChange={setField('role')}
