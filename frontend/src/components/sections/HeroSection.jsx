...
export default function HeroSection() {
  return (
    <section className="min-h-screen grid" style={{ gridTemplateColumns: '55% 45%' }}>
      ...
      {/* ── Gauche ── */}
      <div className="relative flex flex-col justify-center bg-yoba-white grid-pattern"
           style={{ padding: '140px 72px 80px 80px' }}>
        <div className="absolute inset-0 pointer-events-none grid-pattern" />

        <Reveal delay={0}>
          <div className="section-label mb-7">Covoiturage de colis · Architecture microservices</div>
        </Reveal>
        <Reveal delay={0.1}>
          <h1 className="font-condensed font-extrabold uppercase text-yoba-charcoal mb-8 leading-[0.95]"
              style={{ fontSize: 'clamp(52px,6vw,86px)', letterSpacing: '-0.01em' }}>
            Vos colis<br /><em className="not-italic text-brown-light">voyagent</em><br />autrement
          </h1>
        </Reveal>
        <Reveal delay={0.2}>
        ...
        <Reveal delay={0.3}>
          <div className="flex items-center gap-5 flex-wrap mb-16">
            ...
            <a href="#fonctionnement" className="btn-outline">Voir comment ↓</a>
          </div>
        </Reveal>
        <Reveal delay={0.4}>
          <div className="flex gap-12 pt-10 border-t border-brown/10">
            {[{ num:'12K+', lbl:'Colis livrés' }, { num:'4,8★', lbl:'Note moyenne' }, { num:'−65%', lbl:'vs La Poste' }].map(s => (
              <div key={s.lbl}>
                <div className="font-condensed font-extrabold text-brown leading-none mb-1"
                     style={{ fontSize: 40, letterSpacing: '-0.02em' }}>{s.num}</div>
                <div className="text-[11px] font-bold tracking-[0.08em] uppercase text-yoba-gray-dark">{s.lbl}</div>
              </div>
            ))}
          </div>
        </Reveal>
      ...
      <div className="relative bg-brown overflow-hidden flex items-center justify-center">
        ...
        {/* Grand Y décoratif */}
        <div className="absolute right-[-40px] top-1/2 -translate-y-[55%] font-condensed font-extrabold
                        leading-none pointer-events-none select-none"
             style={{ fontSize: 500, color: 'rgba(255,255,255,0.04)' }}>Y</div>

        {/* Carte flottante haut */}
        <motion.div
          className="absolute bg-yoba-white rounded-xl p-4 shadow-card-lg z-10"
          style={{ top: '16%', left: '5%' }}
        ...
        >
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}>
            <p className="text-[10px] font-bold tracking-widest uppercase text-yoba-gray-dark">Trajet disponible</p>
            ...
            <div className="inline-flex items-center gap-1.5 mt-2 bg-brown-pale text-brown text-[10px] font-bold px-2.5 py-1 rounded-full">
              <motion.span animate={{ opacity: [1, 0.2, 1] }} transition={{ duration: 2, repeat: Infinity }}
                           className="text-[6px]">●</motion.span>
              En route
            </div>
          </motion.div>
        </motion.div>
        ...
        {/* Route SVG animée */}
        <svg viewBox="0 0 300 420" fill="none" className="relative z-[3] w-[75%] max-w-[340px]">
          ...
          <path d="M150 50 C165 110 105 145 90 180 C70 220 140 250 210 270 C240 280 200 330 150 370"
          ...
          <g>
            <animateMotion dur="2.8s" repeatCount="indefinite"><mpath href="#rp" /></animateMotion>
        ...
        >
          <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 7.5, repeat: Infinity, ease: 'easeInOut' }}>
            <p className="text-[10px] font-bold tracking-widest uppercase text-yoba-gray-dark">Livraison confirmée ✓</p>
