...
export default function RegisterPage() {
  ...
  const ROLES = [
    ...
    { value: 'both',    label: "🔄 Les deux",                 desc: 'Expéditeur & transporteur' },
  ]
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="hidden lg:flex flex-col justify-between bg-brown p-10 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <YobaleemaLogo size={32} textSize={19} variant="light" />
        </div>
        <div className="relative z-10">
          ...
          <p className="text-sm font-light" style={{ color: 'rgba(255,255,255,0.55)' }}>
            Gratuit, sans engagement. Votre premier envoi en 5 minutes.
          </p>
        </div>
      ...
      <div className="flex flex-col justify-center bg-yoba-gray px-8 py-12 lg:px-20 overflow-y-auto">
        <Link to="/" className="inline-flex items-center gap-2 text-sm text-yoba-gray-dark
                                hover:text-brown transition-colors mb-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
          ...
          <p className="text-sm font-light text-yoba-gray-dark mb-6">
            Créez votre compte Yobaleema gratuitement
          </p>

          {/* Tabs */}
          <div className="flex bg-white border border-brown/10 rounded-lg p-1 mb-6">
            <Link to="/connexion"
                  className="flex-1 py-2.5 text-center text-yoba-gray-dark font-condensed font-bold
                             text-sm tracking-widest uppercase hover:text-brown transition-colors">
              Connexion
            </Link>
            <button className="flex-1 py-2.5 rounded-md bg-brown text-white font-condensed font-bold
          ...
          {error && (
            <div className="mb-5 px-4 py-3 bg-red-50 border border-red-200 rounded-lg
                            text-sm text-red-600 font-medium">{error}</div>
          )}
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Prénom" error={errors.firstName?.message}>
                <Input {...register('firstName')} type="text" placeholder="Aminata" autoComplete="given-name" error={errors.firstName?.message} />
              </Field>
              <Field label="Nom" error={errors.lastName?.message}>
                <Input {...register('lastName')} type="text" placeholder="Koné" autoComplete="family-name" error={errors.lastName?.message} />
              </Field>
            </div>

            <Field label="Adresse email" error={errors.email?.message}>
              <Input {...register('email')} type="email" placeholder="vous@example.com" autoComplete="email" error={errors.email?.message} />
            </Field>

            <Field label="Mot de passe" error={errors.password?.message}>
              <Input {...register('password')} type="password" placeholder="8 caractères minimum" autoComplete="new-password" error={errors.password?.message} />
            </Field>
            ...
            <div className="mb-5">
              ...
              <div className="grid gap-2">
                {ROLES.map(r => {
                  const el = register('role')
                  return (
                    <label key={r.value}
                           className="flex items-center gap-3 p-3 bg-white border border-brown/15
                                      rounded-lg cursor-pointer hover:border-brown transition-colors
                                      has-[:checked]:border-brown has-[:checked]:bg-brown-ultra">
                      <input type="radio" value={r.value} {...el} className="accent-brown" />
                      <div>
